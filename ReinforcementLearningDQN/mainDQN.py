...
import pickle
import csv
from game_envDQN import GameEnv
...
def game_loop():
    ...
    state_size = len(env.reset())  # Convert board state into a NumPy array
    action_size = env.num_positions

    # Create two agents (White and Black)
    white_agent = DQNAgent(state_size, action_size, name="White")
    black_agent = DQNAgent(state_size, action_size, name="Black")

    # white_agent = DQNAgent()
